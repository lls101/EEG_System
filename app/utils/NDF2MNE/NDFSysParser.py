import struct,os
from .NDFSysUtility import FolderCatergory, Subject,Host,Segment,Channel,Event,NDFUtility
import numpy as np
...
class FileNDF:
    ...
    def readAll(self):
        try:
                    ...
                    for j in range(chCount):
                        ...
                        if chTmp.MinDigital >= 0 and width<4:
                            ...
                            if width == 3:
                                ...
                                if(self.IsLE):
                                    # **核心修改：先将数组转为64位整数，再进行计算，确保安全**
                                    b0 = chDataD_U_rsh[:,0].astype(np.int64)
                                    b1 = chDataD_U_rsh[:,1].astype(np.int64)
                                    b2 = chDataD_U_rsh[:,2].astype(np.int64)
                                    chDataDT = b0 + b1 * 256 + b2 * 65536
                            ...
                            if width == 3:
                                ...
                                if(self.IsLE):
                                    ...
                                    b0 = chDataD_U_rsh[:,0].astype(np.int64)
                                    b1 = chDataD_U_rsh[:,1].astype(np.int64)
                                    b2 = chDataD_I_rsh[:,2].astype(np.int64)
                                    chDataDT = b0 + b1 * 256 + b2 * 65536
                                    chDataD = np.reshape(chDataDT,(-1,1))
                                else:
                                    # **核心修改：先将数组转为64位整数，再进行计算，确保安全**
                                    b0 = chDataD_I_rsh[:,0].astype(np.int64)
                                    b1 = chDataD_U_rsh[:,1].astype(np.int64)
                                    b2 = chDataD_U_rsh[:,2].astype(np.int64)
                                    chDataDT = b2 + b1 * 256 + b0 * 65536
                                    chDataD = np.reshape(chDataDT,(-1,1))
                        ...
                        if chTmp.Unit == 'uV' or chTmp.Unit == 'uv':
                            chDataPNp = chDataPNp*1.0e-6
                        elif chTmp.Unit == 'mV' or chTmp.Unit == 'mv':
                            chDataPNp = chDataPNp*1.0e-3
            ...
            # 改进的错误处理，会打印完整的错误信息
            import traceback
            print(f'Failed to read data from {self.filePath}')
            traceback.print_exc()
            ...
            with open(self.filePath,'rb') as fPt:
                ...
                for s in range(duration):
                    ...
                    for j in range(chCount):
                        ...
                        if chTmp.Unit == 'uV' or chTmp.Unit == 'uv':
                            chDataDNp = chDataDNp*1.0e-6
                        elif chTmp.Unit == 'mV' or chTmp.Unit == 'mv':
                            chDataDNp = chDataDNp*1.0e-3
            ...
            with open(self.filePath,'rb') as fPt:
                ...
                for s in range(duration):
                    for j in range(chCount):
                        ...
                        chDataDNp = np.squeeze(chDataD)
                        chDataDNp = chObj.MinPhysical + chObj.D2PCoef*(chDataDNp-chObj.MinDigital)
                        #chDataD = [int.from_bytes(chRawData[width*v:(v+1)*width],bOrder) for v in range(sCount)]
                        #chdataP = [chObj.MinPhysical + chObj.D2PCoef*(v-chObj.MinDigital) for v in chDataD]
                        if chNameTmp.Unit == 'uV' or chNameTmp.Unit == 'uv':
                            chDataDNp = chDataDNp*1.0e-6
                        elif chNameTmp.Unit == 'mV' or chNameTmp.Unit == 'mv':
                            chDataDNp = chDataDNp*1.0e-3
